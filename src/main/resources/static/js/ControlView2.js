$(function(){
    $(".groupTitle").click(function(){
        $(this).next().slideDown(200).parent().siblings().children(".div1").slideUp(200).siblings(".div2").slideUp(200);
    });
    $(".RealTimeli").click(function () {
        $("#RealTimeDetail").slideDown(200);
    });
    $('.labelcheck').click(function () {
        $("#HistoryDetail").slideDown(200);
    });
});
var app;
var dojoConfig = {
...
require([
    ...
    "dojo/domReady!"
             ...
             GraphicsLayer, Slide, Clock, query, on, Array, CalciteMaps, CalciteMapsArcGIS) {
    app = {
        center: [-40, 40],
        scale: 50000000,
        basemap: "streets",
        id: "1fb910b28381473090399cb6e958d8f4",
        viewPadding: {
            top: 50,
            bottom: 0
        },
   ...
   //实时位置
    $("#YingYan").click(function () {
        realLoc();
        document.getElementById("RealTimeLiMen").innerHTML = jsonCoors.length;
    });
    //历史轨迹
    /*app.sceneView.popup.on("trigger-action", function(event) {
        if (event.action.id === "trackOver") {
            onePolyline();
        }
    });*/

    /*$("#trackOver").click(function () {
        onePolyline();
    });*/
     ...
     * @version:1.0
     */
    function onePolyline(tid) {
        var loc;
        var polyline;
        var lineSymbol;
        var graLine;
        $.get({
            url: "http://localhost:8080/userLoc/find/"+tid,
            dataType: "json",
    ...
    app.sceneView.popup.on("trigger-action", function(event) {
        ...
        if (event.action.id === "trackOver") {
            var tid = event.target.content.graphic.attributes;
            app.sceneView.graphics.removeAll();
            onePolyline(tid);
        }
    });
    ...
    /** ---------------------------------------------------
     * @Description: TODO(WebSocket消息通信前端代码)
     * @auther: qzh
